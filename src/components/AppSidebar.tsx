...
const nav = [
  ...
  { href: "/history", label: "Chat history", icon: IconHistory },
  { href: "/summary", label: "Summary", icon: IconChart },
] as const;
  ...
  return (
    <aside className={asideClass} aria-label="Main navigation">
      <Link
        href="/dashboard"
        onClick={() => onNavigate?.()}
        className="flex h-16 items-center gap-2.5 border-b border-slate-100 px-5"
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-[11px] text-slate-500">Your private workspace</div>
        </div>
      </Link>
            ...
            >
              <item.icon
                className={
                  ...
                  (highlighted ? "text-indigo-600" : "text-slate-500 group-hover:text-slate-700")
                }
              />
              {item.label}
              {highlighted ? (
                <span className="ml-auto h-2 w-2 rounded-full bg-indigo-500" aria-hidden />
              ) : null}
            </Link>
          );
        })}
      ...
      <div className="border-t border-slate-100 p-3">
        ...
        <button
          ...
          onClick={() => signOut()}
          disabled={signingOut}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
        >
...
function IconLogout({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.7} stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M18 10.5h3m-3 0-3-3m3 3-3 3" />
    </svg>
  );
}
